 ...
 * A base to build off not, modify before use
 * Author : Alec Wallace
 */ 
...
int TickFct_check(int state){
	...
	switch(state){
		...
		case wait:
			...
			else{//read in data to be transmitted
				while(temp != 0xFF){// end bit
					while(!USART_HasReceived(1)){
						;
					}
					temp = USART_Receive(1);
					packet[packetLength] = temp;
					packetLength++;
				}
				while(!USART_HasReceived(1)){
					;
				}
				numPacketsToSend = (int)USART_Receive(1);
				sendPacket = 0x01;
			}
		break;
		case load:
